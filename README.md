...
Ответ содержит версию `0.6.3` и путь к установленному runtime, поэтому старый Core на порту 1380 не принимается за текущий.
...
Скрипт создаёт build-only Python-окружение, устанавливает закреплённые версии из `requirements-build.txt`, запускает Rust-тесты, собирает GUI и автономный Core, затем создаёт Inno Setup установщик `dist\DelegatorSetup-0.6.3.exe`.

Полезные проверки:

```powershell
cargo test --locked
.\scripts\audit-release.ps1 -InstallerPath .\dist\DelegatorSetup-0.6.3.exe
