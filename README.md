...
**Ваш дорогой ИИ-ассистент тратит токены на работу, которую спокойно сделает кто-то другой.**

Delegator — небольшое приложение для Windows 10/11, живущее в трее. Оно тихо встраивается в ваши
IDE-ассистенты (Codex, Antigravity, OpenCode, Cursor, Claude, VS Code) и забирает у них ту часть
...
Ответ содержит версию `0.4.4` и путь к установленному runtime, поэтому старый Core на порту 1380 не принимается за текущий.
...
Скрипт создаёт build-only Python-окружение, устанавливает закреплённые версии из `requirements-build.txt`, запускает Rust-тесты, собирает GUI и автономный Core, затем создаёт Inno Setup установщик `dist\DelegatorSetup-0.4.4.exe`.

Полезные проверки:

```powershell
cargo test --locked
.\scripts\audit-release.ps1 -InstallerPath .\dist\DelegatorSetup-0.4.4.exe
